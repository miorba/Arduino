...
void setup()
{
  ...
  // Lee las variables guardadas en la EPPROM
  ReadFromEEPROM();
  lertpaEEPROM();
  lerPHAEEPROM();
  lerPHREEPROM();
  ...
  k = EEPROM.read(840); // Puntero que indica que la configuración ya ha cambiado
  ...
  while (!SD.begin(ChipSelect_SD, SPI_QUARTER_SPEED)) // Inicia a comunicação com o cartão SD.
  {
#ifdef USE_TFT //Do not change this line    
    setFont(LARGE, 255, 0, 0, 0, 0, 0);
...
#ifdef USE_STAMP_FOR_PH_AQUARIUM //Do not change this line
...
//  myPinMode(Co2Pin, OUTPUT);                 // Pin 7 modificado por Miguel Ortega: miorba@gmail.com
  ...
  myPinMode(sensor2, INPUT);                 // Pino A1 (Removido);
  ...
  myPinMode(Co2Pin,OUTPUT);                    // Pino 7;
#endif // Do not change this line!
}
