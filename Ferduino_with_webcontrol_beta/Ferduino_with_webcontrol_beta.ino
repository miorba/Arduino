...
//*************************************************************************************************
//Comenta los accesorios que no tienes instalados en tu acuario y que quieres que controlemos
#define USE_Temp_Disp
#define USE_Temp_Water
#define USE_PHAquarium
//#define USE_PHReator
//#define USE_Density
//#define USE_ORP
#define USE_Cooler
...
//#define USE_CPA
#define USE_Temp_Ambient

// Activar o desactivar luces led
// Activa o desactiva las luces led que tengas instaladas en tu acuario
...
//#define USE_STAMP_FOR_PH_AQUARIUM // Comente esta linha se você não tem um "stamp" para o pH do aquário.

// Comment the line below if you haven't a stamp for tank pH.
//#define USE_STAMP_FOR_TANK_PH // Comente esta linha se você não tem um "stamp" para o pH do tanque do aquário.
...
const byte ledPinUV = 9;         // Pino que liga os leds violeta
const byte ledPinBlue = 10;       // Pino que liga os leds azuis
const byte ledPinWhite = 11;     // Pino que liga os leds brancos
const byte ledPinRoyBlue = 999;   // Pino que liga os leds "royal blue"
const byte ledPinRed = 999;       // Pino que liga os leds vermelho
const byte fanPin = 14;          // Pino que controla a velocidade das ventoinhas do dissipador
// Pinos 14 e 15 reservados para a porta serial 3 que se comunica com os "Stamps"
const byte multiplexadorS0Pin = 999; // Pino S0 de controle dos stamps
const byte multiplexadorS1Pin = 999; // Pino S1 de controle dos stamps
...
const byte aquecedorPin = 42;   // Pino que liga o calentador
const byte chillerPin = 43;     // Pino que liga o enfriador
const byte ledPinMoon = 12;     // Pino que liga os leds da luz noturna
const byte wavemaker1 = 84;     // Pino que controla o wavemaker 1
const byte wavemaker2 = 83;     // Pino que controla o wavemaker 2
const byte Co2Pin = 7; // Pino que liga o CO2 del Acuario
...
const byte ozonizadorPin = 999;  // Pino que liga o ozonizador
const byte reatorPin = 999;      // Pino que liga o CO2 do reator.
#endif // Do not change this line!
//***************************************************************************************************
//***************************************************************************************************
...
const byte sensor2 = 999;   // A1; (antes 55)     // Pino analógico que verifica se há tensão na bóia inferior do sump.
...
//***************************************************************************************************
#ifndef USE_PCF8575 // Do not change this line!
const byte bomba1Pin = 60; // ƒ       // Bomba que tira água da quarentena.
#ifndef USE_ESP8266 // Do not change this line!
...
DeviceAddress sensor_ambiente = {0x28, 0x6D, 0x70, 0x2B, 0x04, 0x00, 0x00, 0x80 }; // Es necesario cambiar este valor acorde con nuestro sensor.
...
DeviceAddress sonda1= {0x28, 0x10, 0x32, 0x2B, 0x04, 0x00, 0x00, 0x38 }; // Endereço temporário da sonda1.
DeviceAddress sonda2 = {0x28, 0x99, 0x47, 0x2B, 0x04, 0x00, 0x00, 0xFB }; // Endereço temporário da sonda2.
DeviceAddress sonda3 = {0x28, 0x6D, 0x70, 0x2B, 0x04, 0x00, 0x00, 0x80 }; // Endereço temporário da sonda3.
...
//bit 0;   // Señala que el enfriador está encendido / apagado
//bit 1;   // Señala que el calentador está encendido o apagado
//bit 2;   // Señala que la alarma de temperatura está activada / desactivada
//bit 3;   // Señala que el PH del acuario está fuera de lo especificado
//bit 4;   // Señala que la densidad está fuera de lo especificado
//bit 5;   // Señala que el CO2 está activado
//bit 6;   // Señala que el PH del reactor de calcio está fuera de lo especificado
//bit 7;   // Señala que el ozonizador está encendido / apagado
byte status_parametros_1 = 0x0;
//bit 0    // Señala que el ORP está fuera de lo especificado

byte outlets[9];
//outlets[0] = Calentador > 0 = auto, 1 = on, 2 = off
//outlets[1] = Enfriador > 0 = auto, 1 = on, 2 = off
//outlets[2] = Ozonizador > 0 = auto, 1 = on, 2 = off
//outlets[3] = Reactor de cálcio > 0 = auto, 1 = on, 2 = off
...
//outlets[8] = Reemplazo (ATO) > 0 = auto, 1 = on, 2 = off
//outlets[9] = Co2 > 0 = auto, 1 = on, 2 = off
...
float offTempC = 0.5;          // Variación permitida de temperatura
...
//*****************************************************************************************
static float PHA = 0;              // PH del acuario
float setPHA = 0;           // PH deseado del acuario
float offPHA = 0;           // Variación permitira del PH del acuario
float alarmPHA = 0;         // Variación para accionar la alarma de PH del acuario
int initCo2 = 0;
float CalibracionPHA = 0;
int phaloop=1;  //Loop para hacer media del valor entre 12 valores
float valorpha = 0; // Valor recogido en cada muestreo (está configurado para hacer 12 pasadas y calcular la media)

//#define SIMULARPH // Comentar para simular el funcionamiento de PH, cogerá por defecto el valor 5
//*****************************************************************************************
...
//*****************************************************************************************
int DEN = 0;                 // Densidad
int setDEN = 1025;           // Densidad deseada
byte offDEN = 2;             // Variación permitida para la densidad
byte alarmDEN = 1;           // Variación para accionar la alarma de Densidad

//*****************************************************************************************
//************************ Variáveis de controle do PH do reator de cálcio ****************
//*****************************************************************************************
float PHR = 5;              // Valores PH del reactor
float setPHR = 0;           // PH del reactor deseado
float offPHR = 0;           // Variación permitida PH del reactor
float alarmPHR = 0;         // Variación para accionar la alarma de PH del reactor de Calcio
...
int setORP = 420;           // ORP deseada
byte offORP = 10;           // Variación permitida del ORP
byte alarmORP = 10;         // VVariación para accionar la alarma de ORP
...
#else //Do not change this line
float HtempMin = 30.5;    // Declara la temperatura para iniciar el funcionamento de los ventoinhas del disipador
float HtempMax = 40.5;    // Declara que los ventiladores deben estar en su velocidad máxima cuando eldisipador esté a 40°c
#endif //Do not change this line
int fanSpeed = 0;         // PWM de los ventiladores

//*****************************************************************************************
//************** Variáveis temperárias de controle de velocidade dos coolers **************
//*****************************************************************************************
float HtempMin_temp = 0;    // Declara la temperatura para iniciar el funcionamiento de los ventiladores del disipador
float HtempMax_temp = 0;    // Declara que los ventiladores deben estar a su velocidad máxima cuando el disipador esté a 40ºC

//*****************************************************************************************
//*********************** Variáveis de controle da temperatura do dissipador **************
//*****************************************************************************************
float tempH = 0;    // Temperatura del disipador
byte tempHR = 60;   // Temperatura para reducir la potência de los leds
byte potR = 30;     // Porcentaje a ser reducido

//*****************************************************************************************
//*********** Variáveis temporárias de controle da temperatura do dissipador **************
//*****************************************************************************************
float temperatura_dissipador_temp = 0; // Temperatura temporal del disipador
byte tempHR_t = 0;                     // Temperatura temporal para reduzir la potencia de los leds
byte potR_t = 0;                       // Porcentaje temporal a ser reducido
boolean temperatura_alta = false;      // Señal de que la temperatura de los leds está alta
boolean temperatura_baixou = false;    // Señala que la temperatura de los leds estuvo alta

//*****************************************************************************************
//************************ Variáveis de controle da iluminação ****************************
//*****************************************************************************************
int LedChangTime = 0;             // Página de cambio de leds, tiempo y valores
boolean MeanWell = true;          // Si está utilizando controladores cuya potencia máxima se obtiene aplicando cero voltios y la mínima es 5 voltios cambie de "true" a "false".
boolean LEDtestTick = false;      // Acelerar el tiempo durante la prueba de los leds.
int min_cnt ;
byte bled_out;
byte wled_out;
byte rbled_out;
...
float suavizar = 0.0; // LEDS se inician suavemente y llega al valor especificado en 50 segundos.
byte predefinido = 0;
...
//*****************************************************************************************
String LP;
byte MaxI = 20; // Potencia máxima de la luna llena
byte tMaxI;
byte MinI = 0;   // Potencia mínima de la luna nueva
byte tMinI;
byte fase = 0;
...
//*****************************************************************************************
#ifdef ETHERNET_SHIELD //Do not change this line
const char *Username  = "miorba";           // Coloque aqui o nome de usuário cadastrado no ferduino.com/webcontrol
const char *APIKEY = "cc75b315";                     // Cole aqui a ApiKey gerada pelo ferduino.com/webcontrol
...
//*****************************************************************************************
#define SensorPin A1            //pH meter Analog output to Arduino Analog Input 14
