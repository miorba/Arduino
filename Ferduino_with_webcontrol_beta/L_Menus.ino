...
void graficoScreen()//--------------------------GRAFICOS----------------------tela =9
{
...
void config_phA_Screen(boolean refreshAll = false)
{
  ...
  if (refreshAll)
  {
    ...
    strcpy_P(buffer, (char*)pgm_read_word_near(&(tabela_textos[262])));
    myGLCD.print(buffer, 110, 170); // "CALIBRACION"
    ...
    printButton("-", almM[0], almM[1], almM[2], almM[3], true);      //alarm minus
    printButton("+", almP[0], almP[1], almP[2], almP[3], true);      //alarm plus
    printButton("-", calM[0], calM[1], calM[2], calM[3], true);      //Calibracion menos
    printButton("+", calP[0], calP[1], calP[2], calP[3], true);      //Calibracion mas
  ...
  myGLCD.printNumF(PHA2beC, 2, 120, 190, '.', LEFT, ' ');
}
