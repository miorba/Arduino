...
// comenta los componentes del acuario que utilizas
...
// descomenta cada línea según el tipo de luz que utilices, comenta las que no uses
#define USE_UV
#define USE_Blue
#define USE_White
#define USE_RoyBlue
...
const byte desativarFanPin = 1;   // Pino que desativa os coolers. // Pin que activa los ventiladores
...
const byte ledPinBlue = 70;       // Pino que liga os leds azuis
const byte ledPinWhite = 10;     // Pino que liga os leds brancos
const byte ledPinRoyBlue = 71;   // Pino que liga os leds "royal blue"
const byte ledPinRed = 72;       // Pino que liga os leds vermelho
...
const byte multiplexadorS0Pin = 62; // Pino S0 de controle dos stamps
const byte multiplexadorS1Pin = 63; // Pino S1 de controle dos stamps
...
const byte aquecedorPin = 12;   // Pino que liga o aquecedor // Pin que conecta el calentador
const byte chillerPin = 13;     // Pino que liga o chiller // Pin que conecta el enfriador
const byte ledPinMoon = 9;     // Pino que liga os leds da luz noturna // Pin que conecta la luz nocturna
const byte wavemaker1 = 60;     // Pino que controla o wavemaker 1 / Pin que controla el generador de olas 1
const byte wavemaker2 = 61;     // Pino que controla o wavemaker 2 / Pin que controla el generador de olas 2
...
const byte ozonizadorPin = 75;  // Pino que liga o ozonizador / Pin que conecta el ozonizador
const byte reatorPin = 76;      // Pino que liga o CO2 do reator. / Pin que conecta el CO2 del reactor
#endif // Do not change this line!
//***************************************************************************************************
//***************************************************************************************************
...
const byte sensor1 = 54;   // A0;      // Pino analógico que verifica se há tensão na bóia da quarentena. / Pin analógico que comprueba si hay tensión en la boya de la cuarentena
const byte sensor2 = 55;   // A1;      // Pino analógico que verifica se há tensão na bóia inferior do sump. / Pin analógico que comprueba si hay tensión en la boya inferior del sump
const byte sensor3 = 56;   // A2;      // Pino analógico que verifica se há tensão na bóia superior do sump. / Pin analógico que comprueba si hay tensión en la buya superior del sump
const byte sensor4 = 57;   // A3;      // Pino analógico que verifica se há tensão na bóia inferior do reservatório. / Pin analógico que comprueba si hay tensión en la boya inferior del depósito
const byte sensor5 = 58;   // A4;      // Pino analógico que verifica o nível do reef. / Pin analógico que comprueba el nivel del reef
const byte sensor6 = 59;   // A5;      // Pino analógico que verifica o nível do fish only. / Pin analógico que comprueba el nivel de solo peces
...
#ifdef USE_PCF8575 // Do not change this line!
const byte ozonizadorPin = 999;       // P0       // Pino que liga o ozonizador
const byte reatorPin = 999;           // P1       // Pino que liga o CO2 do reator.
...
const byte skimmerPin = 999;         // P12      // Pino que controla o skimmer
#endif // Do not change this line!
//****************************************************************************************************
...
byte pinoLED[NUMERO_CANAIS] = {ledPinWhite, ledPinUV}; // Branco, azul, azul royal, vermelho, violeta, luz noturna
...
byte ip[] = {192, 168, 1, 177};                     // Configure o IP conforme a sua rede local.
